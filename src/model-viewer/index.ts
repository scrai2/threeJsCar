...
export class ThreeJSComponent {
  ...
  private addGUI(): void {
    ...
    const envControls = {
      ...
      'Tone Mapping': 'Linear', // Default tone mapping function
    };
  ...
  private updateHDRILightingIntensity(intensity: number): void {
    // Ensure this.envMap is not null before applying it to the scene
    if (this.envMap) {
      ...
      // Traverse the scene and update the intensity of any MeshStandardMaterial
      this.scene.traverse((child) => {
        if (child instanceof THREE.Mesh) {
          const material = child.material as THREE.MeshStandardMaterial;
          if (material.envMap) {
